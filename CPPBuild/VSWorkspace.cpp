...
{
	VSWorkspace workspace(setup, packages, workDir, cppbuildDir, vsversion);
	workspace.generate();
}

VSWorkspace::VSWorkspace(const BuildSetup& setup, PackageManager* packages, const std::string& workDir, const std::string& cppbuildDir, const std::string& vsversion)
	: setup(setup), packages(packages), workDir(workDir), cppbuildDir(cppbuildDir), vsversion(vsversion)
{
}
...
void VSWorkspace::createProjectGuids()
{
...
std::string VSWorkspace::getActivePlatform()
{
...
void VSWorkspace::addSolutionFolders()
{
...
void VSWorkspace::addCPPBuildCheck()
{
	std::string projectName = "CPPBuildCheck";
	...
	for (const std::string& item : configureFiles)
	{
		...
		std::string name = FilePath::combine(setup.sourcePath, item);
...
void VSWorkspace::addTargetProject(const BuildTarget& targetDef)
{
